...
class AgentNode(Node):
    ...
    def __init__(self, use_learned_policy=False):
        ...
        self.custom_resolution = 0.6  # meters per "step" (grid cell size)
        self.objects = []
        self.locations = []
        self.obj = []
        self.map = None
        ...
        self.CLASS_TO_IDX = {"ball": 6, "bottle": 7}
    ...
    def agent_call(self):
        ...
        if self.map and (self.is_navigating is False):
            ...
            actions, _, _, _, self.next_lstm_state = self.agent.get_action_and_value(
                next_obs, self.next_lstm_state, next_done, mask=mask
            )
            ...
            if self.use_learned_policy and self.map_ready:
                ...
                if command in [0, 1]:  # left/right
                    ...
                    # done / other actions: do nothing
                    self.get_logger().info('Object found / Done (no navigation).')
                    return
